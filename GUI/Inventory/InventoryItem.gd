...
var dimentions := Vector2(1,1):
	set(dim_new):
#		clear_occupation_flags()
		dimentions = dim_new
#		set_occupation_flags()
		Inventory.clear_drop_preview()
		size = dimentions*Inventory.CELL_SIZE - Vector2(6,6)
...
#		set_occupation_flags()
		Inventory.clear_drop_preview()
		position = grid_position * Inventory.CELL_SIZE + Vector2(4,10)

var is_rotated := false:
	set(rotated):
		is_rotated = rotated
		if is_rotated:
			drag_preview.size = Vector2(dimentions.y, dimentions.x)*Inventory.CELL_SIZE - Vector2(6,6)
		else:
			drag_preview.size = dimentions*Inventory.CELL_SIZE - Vector2(6,6)
